...
router.get("/invoices", authMiddleware, async (req, res) => {
  try {
    ...
    if (establishmentId) conditions.push(eq(invoicesTable.establishmentId, parseInt(establishmentId as string)));
...
router.post("/invoices/:id/pay", authMiddleware, async (req, res) => {
  try {
    ...
    const [invoice] = await db.update(invoicesTable).set({ status: "paid", paidAt: new Date().toISOString().split("T")[0] }).where(eq(invoicesTable.id, parseInt(req.params.id))).returning();
