...
const URGENCY_COLORS: Record<string, string> = { normal: "bg-gray-100 text-gray-700", urgent: "bg-orange-100 text-orange-800", critical: "bg-red-100 text-red-800" };
      ...
      <CardContent className="p-5">
        <div className="flex items-start justify-between gap-4 mb-5 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            {isRejected && <span className="px-2.5 py-1 bg-red-100 text-red-800 rounded-full text-xs font-bold">Rejeté</span>}
          </div>
        </div>
              ...
              {STEPS.map((step, i) => {
                ...
                return (
                  <div key={step} className="flex flex-col items-center" style={{ width: `${100 / STEPS.length}%` }}>
                    ...
                    <p className={`text-xs mt-2 text-center font-medium ${done ? "text-primary" : "text-muted-foreground/60"}`}>
                      {STEP_LABELS[i]}
                    </p>
                  </div>
                );
        ...
        {req.estimatedDelivery && req.status === "shipped" && (
          <p className="text-xs text-blue-700 font-medium mt-4 bg-blue-50 px-3 py-2 rounded-lg">
...
export default function EstablishmentTracking() {
