...
export default function HospitalRequest() {
  ...
  const [internalRequest, setInternalRequest] = useState<any>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.needsExternalSelection) {
        ...
        setInternalRequest(result.internalRequest);
        setStep("select-centers");
        if (result.internalRequest) {
          toast.info(
            "Stock interne insuffisant. Sélectionnez jusqu'à 3 centres externes.",
          );
        } else {
              ...
              {internalRequest && (
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
                  ...
                  <div className="text-sm text-blue-800">
                    <p className="font-semibold mb-1">Demande interne créée</p>
                    <p>
                      Une demande a été envoyée à votre banque de sang interne.
                      Sélectionnez jusqu'à 3 centres externes supplémentaires.
                    </p>
                  </div>
                </div>
              )}
